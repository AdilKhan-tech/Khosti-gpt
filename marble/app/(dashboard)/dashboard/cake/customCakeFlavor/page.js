...
function CustomCakeFlavorPage() {
  ...
  useEffect(() => {
    if (keywords != "") {
      ...
      const loadCustomCakeFlavors = async () => {
        await fetchAllCustomCakeFlavor();
      };

      loadCustomCakeFlavors();
    }
  ...
  const updateCustomCakeFlavor = (updatedCustomCakeFlavor) => {
    setCustomCakeFlavors((prev) =>
      prev.map((customCakeFlavor) =>
        customCakeFlavor.id === updatedCustomCakeFlavor.id
          ? { ...customCakeFlavor, ...updatedCustomCakeFlavor }
          : customCakeFlavor,
      ),
    );
    setShowOffcanvas(false);
                  ...
                  {customCakeFlavors.map((customCakeFlavor, index) => (
                    <tr key={`${customCakeFlavor?.id}-${index}`}>
                      ...
                      <td className="fw-normal fs-14 fnt-color">
                        <div
                          className={
                            customCakeFlavor?.status === "active"
                              ? "blue-status"
                              : "red-status"
                          }
                        >
                      ...
                      <td>
                        <div className="d-flex gap-1">
                          <button
                            ...
                            onClick={() =>
                              showOffcanvasOnEditCustomCakeFlavor(
                                customCakeFlavor,
                              )
                            }
                          >
                          ...
                          <button
                            className="action-btn d-flex justify-content-center align-items-center bg-transparent rounded-2"
                            onClick={() =>
                              showDeleteConfirmation(customCakeFlavor?.id)
                            }
                          >
...
export default CustomCakeFlavorPage;
