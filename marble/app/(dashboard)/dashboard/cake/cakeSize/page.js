...
export default function CakeSizePage() {
  ...
  const updateCakeSize = (updatedCakeSize) => {
    setCakeSizes((prev) =>
      prev.map((cakeSize) =>
        cakeSize.id === updatedCakeSize.id
          ? { ...cakeSize, ...updatedCakeSize }
          : cakeSize,
      ),
    );
    setShowOffcanvas(false);
                    ...
                    <tr key={cakeSize?.id}>
                      ...
                      >
                        <div
                          ...
                          onClick={() => showOffcanvasOnEditCakesSize(cakeSize)}
                        >
                          <i className="bi bi-pencil-square text-primary"></i>
                        </div>
                        <div
                          className="action-btn d-flex justify-content-center align-items-center bg-transparent rounded-2"
                          onClick={() => showDeleteConfirmation(cakeSize.id)}
                        >
