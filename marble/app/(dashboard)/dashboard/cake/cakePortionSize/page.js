...
import {
  getAllCakePortionSizes,
  deleteCakePortionSizeById,
...
export default function CakePortionSizePage() {
  ...
  const handleDelete = async (cakePortionSizeId) => {
    try {
      const response = await axios.delete(
        deleteCakePortionSizeById(cakePortionSizeId),
      );
      if (response.status === 200) {
        toast.success("Custom Cake Type deleted successfully!", {
          autoClose: 1000,
        });
        setRawCakePortionSizes((prev) =>
          prev.filter(
            (cakePortionSize) => cakePortionSize.id !== cakePortionSizeId,
          ),
        );
      }
    } catch (error) {
  ...
  const showDeleteConfirmation = (cakePortionSizeId) => {
    const confirmed = window.confirm(
      "Are you sure you want to delete this Cake Portion Size?",
    );
    if (confirmed) {
                ...
                <tbody>
                  {cakePortionSizes.map((cakePortionSize, index) => (
                    <tr key={cakePortionSize?.id}>
                      ...
                      <td>
                        {cakePortionSize.parent
                          ? cakePortionSize.parent.name_en
                          : "—"}
                      </td>
                      <td className="fw-normal fs-14 fnt-color">
                        <img
                      ...
                      <td className="d-flex gap-2">
                        <div
                          ...
                          onClick={() =>
                            showOffcanvasOnEditCakePortionSizes(cakePortionSize)
                          }
                        >
                        ...
                        <div
                          className="action-btn d-flex justify-content-center align-items-center bg-transparent rounded-2"
                          onClick={() =>
                            showDeleteConfirmation(cakePortionSize.id)
                          }
                        >
            ...
            <Offcanvas.Header closeButton>
              <Offcanvas.Title>
                <div className="fs-24 fnt-color">
                  {cakePortionSizeData
                    ? "Update Cake Portion Size"
                    : "Add Cake Portion Size"}
                </div>
              </Offcanvas.Title>
            </Offcanvas.Header>
            <hr className="mt-0" />
