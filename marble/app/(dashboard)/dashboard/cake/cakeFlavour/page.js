...
export default function CakeFlavourPage() {
  ...
  const handleDelete = async (flavorId) => {
    try {
      ...
      if (response.status === 200) {
        toast.success("Cake flavour deleted successfully!", {
          autoClose: 1000,
        });
        setCakeFlavors((prev) =>
  ...
  const showDeleteConfirmation = (flavorId) => {
    const confirmed = window.confirm(
      "Are you sure you want to delete this Cake flavour?",
    );
    if (confirmed) {
  ...
  const updateCakeFlavor = (updatedCakeFlavor) => {
    setCakeFlavors((prev) =>
      prev.map((cakeFlavor) =>
        cakeFlavor.id === updatedCakeFlavor.id
          ? { ...cakeFlavor, ...updatedCakeFlavor }
          : cakeFlavor,
      ),
    );
    setShowOffcanvas(false);
          ...
          <div className="d-flex justify-content-between mb-3">
            ...
            <div>
              <div
                ...
                onClick={showOffcanvasOnAddCakesFlavour}
                role="button"
              >
                      ...
                      <td>
                        <div className="d-flex gap-1">
                          <button
                            ...
                            onClick={() =>
                              showOffcanvasOnEditCakesFlavour(cakeFlavor)
                            }
                          >
                          ...
                          <button
                            className="action-btn d-flex justify-content-center align-items-center bg-transparent rounded-2 "
                            onClick={() =>
                              showDeleteConfirmation(cakeFlavor.id)
                            }
                          >
