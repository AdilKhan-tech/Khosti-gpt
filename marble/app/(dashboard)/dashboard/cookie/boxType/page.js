...
import {
  getAllCookieBoxTypes,
  deleteCookieBoxTypeById,
...
export default function CookieBoxTypePage() {
  ...
  const handleDelete = async (typeId) => {
    try {
      ...
      if (response.status === 200) {
        toast.success("Cookie box Type deleted successfully!", {
          autoClose: 1000,
        });
  ...
  const showDeleteConfirmation = (typeId) => {
    const confirmed = window.confirm(
      "Are you sure you want to delete this Cookie box type?",
    );
    if (confirmed) {
      handleDelete(typeId);
    }
  ...
  const updateCookieBoxType = (updatedCookieBoxType) => {
    setCookieBoxTypes((prev) =>
      prev.map((cookieBoxType) =>
        cookieBoxType.id === updatedCookieBoxType.id
          ? updatedCookieBoxType
          : cookieBoxType,
      ),
    );
    setShowOffcanvas(false);
                ...
                <tbody>
                  {cookieBoxTypes.map((type, index) => (
                    <tr key={type._id || type.id || index}>
                      <td className="fw-normal fs-14 fnt-color">{type?.id}</td>
                      ...
                      <td className="d-flex gap-2">
                        <div
                          ...
                          onClick={() => showOffcanvasOnEditCookieType(type)}
                        >
                          <i className="bi bi-pencil-square text-primary"></i>
                        </div>
                        <div
                          className="action-btn d-flex justify-content-center align-items-center bg-transparent rounded-2"
                          onClick={() => showDeleteConfirmation(type.id)}
                        >
