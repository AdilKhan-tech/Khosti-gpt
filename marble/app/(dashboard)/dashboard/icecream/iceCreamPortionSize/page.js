...
import {
  getAllIceCreamPortionSizes,
  deleteIceCreamPortionSizeById,
  ...
  const handleDelete = async (iceCreamId) => {
    try {
      const response = await axios.delete(
        deleteIceCreamPortionSizeById(iceCreamId),
      );
      if (response.status === 200) {
        toast.success("Ice Cream portion size deleted successfully!", {
          autoClose: 1000,
        });
        setRawIceCreamPortionSizes((prev) =>
          prev.filter(
            (iceCreamPortionSize) => iceCreamPortionSize.id !== iceCreamId,
          ),
        );
      }
    } catch (error) {
                ...
                <tbody>
                  {iceCreamPortionSizes.map((icecream, index) => (
                    <tr key={`${icecream.id}-${index}`}>
                      ...
                      <td>{icecream.parent ? icecream.parent.name_en : "—"}</td>
                      <td className="fw-normal fs-14 fnt-color">
                        {icecream.slug}
                      </td>
                      <td className="fw-normal fs-14 fnt-color">
